...
def checksend(arg=None):
	if listenorconnect.get() == "LISTEN" and (serverison.is_set() == False or connect_event.is_set() == False):
		...
		ReceivingQueue.put(("No connection exists to send anything through. Establish connection first.","red"))
		return
	tosend = str(SendingText.get("1.0",END))
	# print repr(tosend)
	# print tosend
	if '\n' in tosend[len(tosend)-1:]:
		tosend=tosend[:len(tosend)-1]
	# print tosend
	if tosend == "": # prevent empty strings being processed, sent
		return
	# print tosend
	if "u'" in tosend[0:2] or 'u"' in tosend[0:2]:
		tosend=tosend[2:]
	# print tosend
...
ReceivingText=Text(ReceivingFrame,state=DISABLED, height=15,bd=1,relief=SUNKEN,highlightthickness=0,undo=True, font=("Helvetica",14))
...
SendingText=Text(SendingFrame,height=2,bd=1,relief=SUNKEN,highlightthickness=0,undo=True, font=("Helvetica",14))
